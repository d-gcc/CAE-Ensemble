...
from utils.config import MASConfig
mpl.use('Agg')
...
class MASModel(object):
    ...
    def fit(self, time_series):
        ...
        if self.model_type == 1:
            ...
            df_time_series_rolling = df_time_series.rolling(window=self.window, center=True, min_periods=1).mean()
        # exponential average mean
        elif self.model_type == 2:
            df_time_series = pd.DataFrame(time_series)
            df_time_series_rolling = df_time_series.ewm(alpha=self.alpha, adjust=False).mean()
        return df_time_series_rolling
...
def RunModel(file_name, config):
    ...
    model = MASModel(file_name=file_name, config=config)

    if train_data is not None and config.robustness == False:
        mas_output = model.fit(time_series=abnormal_data)
    ...
    score = np.sum(np.square(mas_output - abnormal_data), axis=1)
    final_zscore = zscore(score)
    ...
    if config.save_figure:
        if original_x_dim == 1:
            ...
            plt.tight_layout()
            # plt.show()
            plt.savefig('./save_figures/{}/Ori_MAS_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()

            # Plot decoder output
            plt.figure(figsize=(9, 3))
            plt.plot(mas_output, color='blue', lw=1.5)
            plt.title('Decoding Output')
            plt.grid(True)
            plt.tight_layout()
            # plt.show()
            plt.savefig('./save_figures/{}/Dec_MAS_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()
            ...
            # plt.show()
            plt.savefig('./save_figures/{}/VisInp_MAS_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()
            ...
            plt.scatter(t, abnormal_data, s=markersize, c=markercolors)
            # plt.show()
            plt.savefig('./save_figures/{}/VisOut_MAS_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()
        else:
    ...
    if config.use_spot:
        ...
        try:
            ...
            if config.save_output:
                ...
                np.save('./save_outputs/NPY/{}/{}#{}#{}#{}.npy'.format(config.dataset, model_name, Path(file_name).stem, config.pid, config.window), score)
...
if __name__ == '__main__':
    ...
    if args.load_config:
        config = MASConfig(dataset=None, x_dim=None, model_type=None, window=None, alpha=None, display=None,
                           save_output=None, save_figure=None, use_spot=None, save_config=None, load_config=None,
                           server_run=None, robustness=None, adjusted_points=None, pid=None)
        try:
            config.import_config('./save_configs/{}/Config_MAS_pid={}.json'.format(config.dataset, config.pid))
        except:
            print('There is no config.')
    else:
        config = MASConfig(dataset=args.dataset, x_dim=args.x_dim, model_type=args.model_type, window=args.window,
                           alpha=args.alpha, display=args.display, save_output=args.save_output,
                           ...
                           adjusted_points=args.adjusted_points,pid=args.pid)

    if args.save_config:
        if not os.path.exists('./save_configs/{}/'.format(config.dataset)):
            os.makedirs('./save_configs/{}/'.format(config.dataset))
        config.export_config('./save_configs/{}/Config_MAS_pid={}.json'.format(config.dataset, config.pid))
    ...
    train_logger, file_logger, meta_logger = create_logger(dataset=args.dataset,
                                                           train_logger_name='mas_train_logger',
                                                           file_logger_name='mas_file_logger',
                                                           meta_logger_name='mas_meta_logger',
                                                           model_name='MAS',
                                                           pid=args.pid)
        ...
        if args.server_run:
            try:
                ...
                send_email_notification(
                    subject='baseline_mas application CRASHED! dataset error: {}, file error: {}, '
    ...
    if args.server_run:
        send_email_notification(
            subject='baseline_moving_average application FINISHED! dataset: {}, pid: {}'.format(args.dataset, args.pid),
            message='baseline_moving_average FINISHED! dataset: {}, pid: {}'.format(args.dataset, args.pid))