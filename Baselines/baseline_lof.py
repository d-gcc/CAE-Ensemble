...
def RunModel(file_name, config):
    ...
    if config.save_figure:
        ...
        if original_x_dim == 1:
            ...
            plt.tight_layout()
            # plt.show()
            plt.savefig('./save_figures/{}/Ori_LOF_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()
            ...
            # plt.show()
            plt.savefig('./save_figures/{}/VisInp_LOF_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()

            markercolors = ['blue' if i == 1 else 'red' for i in lof_output.y_hat]
            markersize = [4 if i == 1 else 25 for i in lof_output.y_hat]
            plt.figure(figsize=(9, 3))
            ax = plt.axes()
            plt.yticks([0, 0.25, 0.5, 0.75, 1])
            ax.set_xlim(t[0] - 10, t[-1] + 10)
            ...
            plt.scatter(t, abnormal_data, s=markersize, c=markercolors)
            # plt.show()
            plt.savefig('./save_figures/{}/VisOut_LOF_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()
        else:
    ...
    if config.use_spot:
        ...
        try:
            ...
            if config.preprocessing:
                for index, lof_output in enumerate(outputs):
                    if config.use_overlapping:
                        if config.use_last_point:
                            np_decision = np.append(np_decision,lof_output.y_hat[-1])
                            score = np.append(score,-lof_output.negative_factor[-1])
                            abnormal_segment = np.append(abnormal_segment,rolling_abnormal_label[index][-1])
                        else:
                            values, counts = np.unique(lof_output.y_hat, return_counts=True)
                            np_decision = np.append(np_decision,values[np.argmax(counts)])
                            score = np.append(score,np.average(-lof_output.negative_factor))
                            abnormal_segment = np.append(abnormal_segment,rolling_abnormal_label[index][-1])
                    else:
                        np_decision = np.append(np_decision,lof_output.y_hat)
                        score = np.append(score,-lof_output.negative_factor)
                        abnormal_segment = np.append(abnormal_segment,rolling_abnormal_label[index])
            else:
                lof_output = outputs[0]
                np_decision = lof_output.y_hat
                score = -lof_output.negative_factor
...
if __name__ == '__main__':
    ...
    if args.load_config:
        config = LOFConfig(dataset=None, x_dim=None, n_neighbors=None, algorithm=None, leaf_size=None, metric=None,
                           ...
                           load_model=None, use_spot=None, save_config=None, load_config=None, use_slope=None, slope=None,
        ...
        try:
            config.import_config('./save_configs/{}/Config_LOF_pid={}.json'.format(config.dataset, config.pid))
        except:
        ...
        config = LOFConfig(dataset=args.dataset, x_dim=args.x_dim, n_neighbors=args.n_neighbors,
                           ...
                           robustness=args.robustness, pid=args.pid, adjusted_points=args.adjusted_points,
    ...
    if args.save_config:
        ...
        config.export_config('./save_configs/{}/Config_LOF_pid={}.json'.format(config.dataset, config.pid))
    # %%
        ...
        if args.server_run:
            try:
                ...
                send_email_notification(
                    subject='baseline_lof application CRASHED! dataset error: {}, file error: {}, '
    ...
    if args.server_run:
        send_email_notification(
            subject='baseline_lof application FINISHED! dataset: {}, pid: {}'.format(args.dataset, args.pid),
            message='baseline_lof application FINISHED! dataset: {}, pid: {}'.format(args.dataset, args.pid))