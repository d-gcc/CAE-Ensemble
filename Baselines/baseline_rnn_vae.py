...
class Encoder(nn.Module):
    def __init__(self, x_dim, h_dim, z_dim, sequence_length, use_bidirection=False, batch_size=1, rnn_layers=1):
    ...
    def forward(self, x):
        ...
        z_std = []
        z = []
        h_i = Variable(torch.zeros(x.shape[0], self.h_dim), requires_grad=True).to(device)
        for i in range(x.shape[1]):
            h_i = self.gru_cell(x[:, i], (h_i))
            z_mean_i = self.enc_mean(h_i)
            z_std_i = self.enc_std(h_i)
            z_i = self.reparameterized_sample(z_mean_i, z_std_i)
            h.append(h_i)
            z_mean.append(z_mean_i)
            z_std.append(z_std_i)
            z.append(z_i)
        return torch.stack(h, dim=1), torch.stack(z_mean, dim=1), torch.stack(z_std, dim=1), torch.stack(z, dim=1)
...
class Decoder(nn.Module):
    def __init__(self, x_dim, h_dim, z_dim, sequence_length, use_bidirection=False, batch_size=1, rnn_layers=1):
        ...
        # units
        self.gru_cell = nn.GRUCell(self.z_dim, self.x_dim, bias=True)

    def forward(self, z):
        x = []
        x_i = Variable(torch.zeros(z.shape[0], self.x_dim), requires_grad=True).to(device)
        for i in range(z.shape[1]):
            x_i = self.gru_cell(z[:, i], (x_i))
            x.append(x_i)
...
class RNNVAE(nn.Module):
    def __init__(self, file_name, config):
        ...
        # units
        self.encoder = Encoder(x_dim=self.x_dim, h_dim=self.h_dim, z_dim=self.z_dim, sequence_length=self.rolling_size,
                               use_bidirection=False, batch_size=self.batch_size, rnn_layers=self.rnn_layers)
        self.decoder = Decoder(x_dim=self.x_dim, h_dim=self.h_dim, z_dim=self.z_dim, sequence_length=self.rolling_size,
                               use_bidirection=False, batch_size=self.batch_size, rnn_layers=self.rnn_layers)
    ...
    def forward(self, input):
        ...
        h, z_mean, z_std, z = self.encoder(input)
        decoded_output = self.decoder(z)
        nll_loss += self.mse(x=input, mean=decoded_output)
        kld_loss += self.kld_gaussian(mean_1=z_mean, std_1=z_std, mean_2=None, std_2=None)
        return decoded_output, z, nll_loss, kld_loss
    ...
    def fit(self, train_input, train_label, test_input, test_label, abnormal_data, abnormal_label, original_x_dim):
        ...
        if self.load_model == True and self.continue_training == False:
            ...
            for epoch in range(self.epochs):
                ...
                for i, (batch_x, batch_y) in enumerate(train_data):
                    ...
                    batch_x_reconstruct, batch_z, nll_loss, kld_loss = self.forward(batch_x)
                ...
                if epoch % self.display_epoch == 0:
                    ...
                    with torch.no_grad():
                        ...
                        for i, (batch_x, batch_y) in enumerate(test_data):
                            ...
                            cat_zs.append(batch_x_reconstruct)
                        # TODO
                        if self.preprocessing:
                            ...
                            cat_zs = torch.cat(cat_zs, dim=2)
                        rnn_vae_output = RNNVAEOutput(dec_means=cat_xs, zs=cat_zs, best_TN=None, best_FP=None,
                                                      best_FN=None, best_TP=None, best_precision=None, best_recall=None,
                                                      best_fbeta=None, best_pr_auc=None, best_roc_auc=None,
                                                      best_cks=None)
                        ...
                        if self.preprocessing:
                            if self.use_overlapping:
                                if self.use_last_point:
                                    dec_mean_unroll = rnn_vae_output.dec_means.detach().cpu().numpy()[:, -1]
                                    dec_mean_unroll = min_max_scaler.fit_transform(dec_mean_unroll)
                                    x_original_unroll = abnormal_data[self.rolling_size - 1:]
                                else:
                                    dec_mean_unroll = unroll_window_3D(rnn_vae_output.dec_means.detach().cpu().numpy())[::-1]
                                    dec_mean_unroll = min_max_scaler.fit_transform(dec_mean_unroll)
                                    x_original_unroll = abnormal_data[: dec_mean_unroll.shape[0]]

                            else:
                                dec_mean_unroll = np.reshape(rnn_vae_output.dec_means.detach().cpu().numpy(), (-1, original_x_dim))
                                dec_mean_unroll = min_max_scaler.fit_transform(dec_mean_unroll)
                                x_original_unroll = abnormal_data[: dec_mean_unroll.shape[0]]
                        else:
                            dec_mean_unroll = rnn_vae_output.dec_means.detach().cpu().numpy()
        ...
        with torch.no_grad():
            ...
            rnn_vae_output = RNNVAEOutput(dec_means=cat_xs, zs=cat_zs, best_TN=max(TN), best_FP=max(FP), best_FN=max(FN), best_TP=max(TP),
...
def RunModel(file_name, config):
    ...
    model = RNNVAE(file_name=file_name, config=config)
    model = model.to(device)
    if train_data is not None and config.robustness == False:
        rnn_vae_output = model.fit(train_input=rolling_train_data, train_label=rolling_train_data,
        ...
        rnn_vae_output = model.fit(train_input=rolling_abnormal_data, train_label=rolling_abnormal_data,
                                   test_input=rolling_abnormal_data, test_label=rolling_abnormal_label,
                                   abnormal_data=abnormal_data, abnormal_label=abnormal_label,
                                   original_x_dim=original_x_dim)
    ...
    if config.preprocessing:
        if config.use_overlapping:
            if config.use_last_point:
                dec_mean_unroll = rnn_vae_output.dec_means.detach().cpu().numpy()[:, -1]
                dec_mean_unroll = min_max_scaler.fit_transform(dec_mean_unroll)
                latent_mean_unroll = rnn_vae_output.zs.detach().cpu().numpy()
    ...
    if config.save_output:
        ...
        np.save('./save_outputs/NPY/{}/Dec_RNNVAE_{}_pid={}.npy'.format(config.dataset, Path(file_name).stem, config.pid), dec_mean_unroll)
        np.save('./save_outputs/NPY/{}/Latent_RNNVAE_{}_pid={}.npy'.format(config.dataset, Path(file_name).stem, config.pid), latent_mean_unroll)
    ...
    if config.save_figure:
        ...
        if original_x_dim == 1:
            ...
            plt.tight_layout()
            # plt.show()
            plt.savefig('./save_figures/{}/Ori_RNNVAE_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()
            ...
            plt.tight_layout()
            # plt.show()
            plt.savefig('./save_figures/{}/Dec_RNNVAE_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()
            ...
            # plt.show()
            plt.savefig('./save_figures/{}/VisInp_RNNVAE_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()
            ...
            # plt.show()
            plt.savefig('./save_figures/{}/VisOut_RNNVAE_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()
        else:
    ...
    if config.use_spot:
        ...
        try:
            ...
            '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', 
            '{}', '{}', '{}')""".format(
                'RNNVAE', config.pid, settings, config.dataset, Path(file_name).stem, cm[0][0], cm[0][1], cm[1][0],
                cm[1][1], precision, recall, fbeta, pr_auc, roc_auc, cks, rnn_vae_output.best_TN, rnn_vae_output.best_FP,
                rnn_vae_output.best_FN, rnn_vae_output.best_TP, rnn_vae_output.best_precision, rnn_vae_output.best_recall,
                rnn_vae_output.best_fbeta, rnn_vae_output.best_pr_auc, rnn_vae_output.best_roc_auc, rnn_vae_output.best_cks, 
                execution_time)
            ...
            metrics_result = MetricsResult(TN=cm[0][0], FP=cm[0][1], FN=cm[1][0], TP=cm[1][1], precision=precision,
                                           ...
                                           best_TN=rnn_vae_output.best_TN, best_FP=rnn_vae_output.best_FP,
                                           best_FN=rnn_vae_output.best_FN, best_TP=rnn_vae_output.best_TP,
                                           best_precision=rnn_vae_output.best_precision, best_recall=rnn_vae_output.best_recall,
                                           best_fbeta=rnn_vae_output.best_fbeta, best_pr_auc=rnn_vae_output.best_pr_auc,
                                           best_roc_auc=rnn_vae_output.best_roc_auc, best_cks=rnn_vae_output.best_cks)
...
if __name__ == '__main__':
    ...
    if args.load_config:
        config = RNNVAEConfig(dataset=None, x_dim=None, h_dim=None, z_dim=None, preprocessing=None,
                              ...
                              rnn_layers=None, lmbda=None, use_clip_norm=None, gradient_clip_norm=None, use_slope=None,
                              use_bidirection=None, force_teaching=None, force_teaching_threshold=None, slope=None,
                              display_epoch=None, save_output=None, save_figure=None, save_model=None, load_model=None,
                              continue_training=None, dropout=None, use_spot=None, use_last_point=None, adjusted_points=None,
                              use_best_f1=None, save_config=None, load_config=None, server_run=None, robustness=None, pid=None)
        try:
            config.import_config('./save_configs/{}/Config_RNNVAE_pid={}.json'.format(config.dataset, config.pid))
        except:
            print('There is no config.')
    else:
        config = RNNVAEConfig(dataset=args.dataset, x_dim=args.x_dim, h_dim=args.h_dim, z_dim=args.z_dim,
                              ...
                              rnn_layers=args.rnn_layers, lmbda=args.lmbda, use_bidirection=args.use_bidirection,
                              force_teaching=args.force_teaching, use_slope=args.use_slope, slope=args.slope,
                              force_teaching_threshold=args.force_teaching_threshold, display_epoch=args.display_epoch,
    ...
    if args.save_config:
        ...
        config.export_config('./save_configs/{}/Config_RNNVAE_pid={}.json'.format(config.dataset, config.pid))
    # %%
    device = torch.device(get_free_device())

    train_logger, file_logger, meta_logger = create_logger(dataset=args.dataset,
                                                           train_logger_name='rnn_vae_train_logger',
                                                           file_logger_name='rnn_vae_file_logger',
                                                           meta_logger_name='rnn_vae_meta_logger',
                                                           model_name='RNNVAE',
                                                           pid=args.pid)
        ...
        if args.server_run:
            try:
                ...
                send_email_notification(
                    subject='baseline_rnn_vae application CRASHED! dataset error: {}, file error: {}, '
    ...
    if args.server_run:
        send_email_notification(
            subject='baseline_rnn_vae application FINISHED! dataset: {}, pid: {}'.format(args.dataset, args.pid),
            message='baseline_rnn_vae application FINISHED! dataset: {}, pid: {}'.format(args.dataset, args.pid))