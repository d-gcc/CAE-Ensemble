                ...
                if epoch % self.display_epoch == 0:
                    ...
                    with torch.no_grad():
                        ...
                        for i, (batch_x, batch_y) in enumerate(test_data):
                            ...
                            batch_x_mean, batch_x_std = self.forward(input=batch_x.to(device))
...
def RunModel(file_name, config):
    ...
    model = OMNIANOMALY(file_name=file_name, config=config)
    ...
    if config.save_output:
        ...
        np.save('./save_outputs/NPY/{}/Dec_OMNIANOMALY_{}_pid={}.npy'.format(config.dataset, Path(file_name).stem, config.pid), dec_mean_unroll)
        np.save('./save_outputs/NPY/{}/Latent_OMNIANOMALY_{}_pid={}.npy'.format(config.dataset, Path(file_name).stem, config.pid), latent_mean_unroll)
    ...
    if config.save_figure:
        ...
        if original_x_dim == 1:
            ...
            #plt.axis([0, 1500, 0, 1])
            # plt.show()
            plt.savefig('./save_figures/{}/Ori_OMNIANOMALY_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()
            ...
            #plt.axis([0, 1500, 0, 1])
            # plt.show()
            plt.savefig('./save_figures/{}/Dec_OMNIANOMALY_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()
            ...
            # plt.show()
            plt.savefig('./save_figures/{}/VisInp_OMNIANOMALY_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()
            ...
            # plt.show()
            plt.savefig('./save_figures/{}/VisOut_OMNIANOMALY_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()
        else:
    ...
    if config.use_spot:
        ...
        try:
            ...
            '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', 
            '{}', '{}', '{}')""".format(
                'OMNIANOMALY', config.pid, settings, config.dataset, Path(file_name).stem, cm[0][0], cm[0][1], cm[1][0],
                cm[1][1], precision, recall, fbeta, pr_auc, roc_auc, cks, omnianomaly_output.best_TN, omnianomaly_output.best_FP,
                omnianomaly_output.best_FN, omnianomaly_output.best_TP, omnianomaly_output.best_precision, omnianomaly_output.best_recall,
                omnianomaly_output.best_fbeta, omnianomaly_output.best_pr_auc, omnianomaly_output.best_roc_auc, omnianomaly_output.best_cks, execution_time)
...
if __name__ == '__main__':
    ...
    if args.load_config:
        config = VRAEConfig(dataset=None, x_dim=None, h_dim=None, z_dim=None, preprocessing=None, use_overlapping=None,
                            ...
                            use_clip_norm=None, gradient_clip_norm=None, use_PNF=None, PNF_layers=None, slope=None,
                            ...
                            robustness=None, adjusted_points=None, use_best_f1=None, use_slope=None, pid=None)
        try:
            config.import_config('./save_configs/{}/Config_OMNIANOMALY_pid={}.json'.format(config.dataset, config.pid))
        except:
        ...
        config = VRAEConfig(dataset=args.dataset, x_dim=args.x_dim, h_dim=args.h_dim, z_dim=args.z_dim,
                            ...
                            lmbda=args.lmbda, use_clip_norm=args.use_clip_norm, use_slope=args.use_slope,
                            gradient_clip_norm=args.gradient_clip_norm, rnn_layers=args.rnn_layers, slope=args.slope,
    ...
    if args.save_config:
        ...
        config.export_config('./save_configs/{}/Config_OMNIANOMALY_pid={}.json'.format(config.dataset, config.pid))
    # %%
        ...
        if args.server_run:
            try:
                ...
                send_email_notification(
                    subject='baseline_omnianomaly application CRASHED! dataset error: {}, file error: {}, '
    ...
    if args.server_run:
        send_email_notification(
            subject='baseline_omnianomaly application FINISHED! dataset: {}, pid: {}'.format(args.dataset, args.pid),
            message='baseline_omnianomaly application FINISHED! dataset: {}, pid: {}'.format(args.dataset, args.pid))