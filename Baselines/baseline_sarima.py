...
class SARIMAModel(object):
    ...
    def fit(self, train_data, abnormal_data):
        ...
        if train_data is not None:
            model = sm.tsa.statespace.SARIMAX(train_data, order=(self.AR, self.I, self.MA),
                                              seasonal_order=(self.S_P, self.S_D, self.S_Q, self.S_s), trend='ct')
            model_fit = model.fit(disp=1)
            output = model_fit.predict(start=0, end=abnormal_data.shape[0] - 1, dynamic=False)
        else:
            model = sm.tsa.statespace.SARIMAX(abnormal_data[0:self.train_window], order=(self.AR, self.I, self.MA),
                                              seasonal_order=(self.S_P, self.S_D, self.S_Q, self.S_s), trend='ct')
            model_fit = model.fit(disp=1)
            output = model_fit.predict(start=0, end=abnormal_data.shape[0] - 1, dynamic=False)
        # else:
...
def RunModel(file_name, config):
    ...
    model = SARIMAModel(file_name=file_name, config=config)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--AR', type=int, default=5)
    parser.add_argument('--MA', type=int, default=5)
    parser.add_argument('--I', type=int, default=1)
    parser.add_argument('--S_P', type=int, default=0)
    parser.add_argument('--S_Q', type=int, default=0)
    parser.add_argument('--S_D', type=int, default=0)
    parser.add_argument('--S_s', type=int, default=0)
    ...
    if args.load_config:
        config = ARMAConfig(dataset=None, x_dim=None, train_window=None, AR=None, MA=None, I=None, S_P=None, S_D=None,
                            S_Q=None, S_s=None, display=None, save_output=None, save_figure=None, use_spot=None,
                            save_config=None, load_config=None, server_run=None, robustness=None, pid=None)
        try:
        ...
        config = ARMAConfig(dataset=args.dataset, x_dim=args.x_dim, train_window=args.train_window, AR=args.AR,
                            MA=args.MA, I=args.I, S_P=args.S_P, S_D=args.S_D, S_Q=args.S_Q, S_s=args.S_s,
