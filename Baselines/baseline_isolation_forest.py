...
from sklearn.metrics import precision_score, recall_score, precision_recall_curve, roc_curve, auc, cohen_kappa_score,     fbeta_score, confusion_matrix
...
#Replicability
random_seed = 0 #https://scikit-learn.org/stable/glossary.html#term-random_state
...
class ISF(object):
    def __init__(self, file_name, config):
        ...
        self.model = IsolationForest(n_estimators=self.n_estimators, max_samples=self.max_samples,
                                     ...
                                     contamination=self.contamination, random_state=random_seed)
...
def RunModel(file_name, config):
    ...
    if config.preprocessing:
        ...
        model = ISF(file_name, config)
        outputs.append(model.fit(train_input=abnormal_data, train_label=abnormal_label, test_input=abnormal_data, test_label=abnormal_label))
    ...
    if config.save_figure:
        ...
        if original_x_dim == 1:
            ...
            plt.tight_layout()
            # plt.show()
            plt.savefig('./save_figures/{}/Ori_ISF_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()
            ...
            # plt.show()
            plt.savefig('./save_figures/{}/VisInp_ISF_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()

            markercolors = ['blue' if i == 1 else 'red' for i in isf_output.y_hat]
            markersize = [4 if i == 1 else 25 for i in isf_output.y_hat]
            plt.figure(figsize=(9, 3))
            ax = plt.axes()
            plt.yticks([0, 0.25, 0.5, 0.75, 1])
            ax.set_xlim(t[0] - 10, t[-1] + 10)
            ...
            plt.scatter(t, abnormal_data, s=markersize, c=markercolors)
            # plt.show()
            plt.savefig('./save_figures/{}/VisOut_ISF_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()
        else:
    ...
    if config.use_spot:
        ...
        try:
            ...
            if config.preprocessing:
                for index, isf_output in enumerate(outputs):
                    if config.use_overlapping:
                        if config.use_last_point:
                            np_decision = np.append(np_decision,isf_output.y_hat[-1])
                            score = np.append(score,-isf_output.decision_function[-1])
                            abnormal_segment = np.append(abnormal_segment,rolling_abnormal_label[index][-1])
                        else:
                            values, counts = np.unique(isf_output.y_hat, return_counts=True)
                            np_decision = np.append(np_decision,values[np.argmax(counts)])
                            score = np.append(score,np.average(-isf_output.decision_function))
                            abnormal_segment = np.append(abnormal_segment,rolling_abnormal_label[index][-1])
                    else:
                        np_decision = np.append(np_decision,isf_output.y_hat)
                        score = np.append(score,-isf_output.decision_function)
                        abnormal_segment = np.append(abnormal_segment,rolling_abnormal_label[index])
            else:
                isf_output = outputs[0]
                np_decision = isf_output.y_hat
                score = -isf_output.decision_function
...
if __name__ == '__main__':
    ...
    if args.load_config:
        config = ISFConfig(dataset=None, x_dim=None, n_estimators=None, max_samples=None, contamination=None,
                           ...
                           adjusted_points=None, use_best_f1=None, robustness=None, pid=None, use_slope=None, 
                           slope=None, preprocessing=None, use_overlapping=None, rolling_size=None, use_last_point=None)
        try:
            config.import_config('./save_configs/{}/Config_ISF_pid={}.json'.format(config.dataset, config.pid))
        except:
        ...
        config = ISFConfig(dataset=args.dataset, x_dim=args.x_dim, n_estimators=args.n_estimators,
                           ...
                           robustness=args.robustness, adjusted_points=args.adjusted_points, use_last_point=args.use_last_point,
                           use_best_f1=args.use_best_f1, pid=args.pid, use_slope=args.use_slope, slope=args.slope,
                           preprocessing=args.preprocessing, use_overlapping=args.use_overlapping, rolling_size=args.rolling_size)
    if args.save_config:
        if not os.path.exists('./save_configs/{}/'.format(config.dataset)):
            os.makedirs('./save_configs/{}/'.format(config.dataset))
        config.export_config('./save_configs/{}/Config_ISF_pid={}.json'.format(config.dataset, config.pid))
    # %%
        ...
        if args.server_run:
            try:
                ...
                send_email_notification(
                    subject='baseline_isf application CRASHED! dataset error: {}, file error: {}, '
    ...
    if args.server_run:
        send_email_notification(
            subject='baseline_isf application FINISHED! dataset: {}, pid: {}'.format(args.dataset, args.pid),
            message='baseline_isf application FINISHED! dataset: {}, pid: {}'.format(args.dataset, args.pid))
