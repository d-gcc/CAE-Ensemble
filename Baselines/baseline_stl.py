...
from utils.config import STLConfig
mpl.use('Agg')
...
from utils.data_provider import read_GD_dataset, read_HSS_dataset, read_S5_dataset, read_NAB_dataset, read_ECG_dataset, \
    ...
    rolling_window_2D, cutting_window_2D, read_SWAT_dataset, read_WADI_dataset
...
class STLModel(object):
    ...
    def fit(self, time_series):
        ...
        for channel in range(time_series.shape[1]):
            time_series_channel = pd.DataFrame(time_series[:, channel])
            component = STL(time_series_channel, period=self.period, robust=self.robust).fit()
            trends.append(component.trend.to_numpy())
            periodicities.append(component.seasonal.to_numpy())
        trends = np.stack(trends, axis=1)
        periodicities = np.stack(periodicities, axis=1)
        output = trends + periodicities
        return output
...
def RunModel(file_name, config):
    ...
    model = STLModel(file_name=file_name, config=config)

    if train_data is not None and config.robustness == False:
        stl_output = model.fit(time_series=abnormal_data)
    elif train_data is None or config.robustness == True:
        stl_output = model.fit(time_series=abnormal_data)

    if config.save_output == True:
        np.save('./save_outputs/NPY/{}/Score_STL_{}_pid={}.npy'.format(config.dataset, Path(file_name).stem, config.pid), stl_output)

    # TODO metrics computation.
    anomaly_score = np.sum(np.square(stl_output - abnormal_data), axis=1)
    ...
    if config.save_figure:
        if original_x_dim == 1:
            ...
            plt.tight_layout()
            # plt.show()
            plt.savefig('./save_figures/{}/Ori_STL_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()

            # Plot decoder output
            plt.figure(figsize=(9, 3))
            plt.plot(stl_output, color='blue', lw=1.5)
            plt.title('Decoding Output')
            plt.grid(True)
            plt.tight_layout()
            # plt.show()
            plt.savefig('./save_figures/{}/Dec_STL_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()
            ...
            # plt.show()
            plt.savefig('./save_figures/{}/VisInp_STL_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()
            ...
            plt.scatter(t, abnormal_data, s=markersize, c=markercolors)
            # plt.show()
            plt.savefig('./save_figures/{}/VisOut_STL_{}_pid={}.png'.format(config.dataset, Path(file_name).stem, config.pid), dpi=300)
            plt.close()
        else:
    ...
    if config.use_spot:
        ...
        try:
            ...
            fpr, tpr, _ = roc_curve(y_true=np.nan_to_num(abnormal_label), y_score=np.nan_to_num(anomaly_score), pos_label=pos_label)
            roc_auc = auc(fpr, tpr)
            pre, re, _ = precision_recall_curve(y_true=np.nan_to_num(abnormal_label), probas_pred=np.nan_to_num(anomaly_score), pos_label=pos_label)
            pr_auc = auc(re, pre)
            cks = cohen_kappa_score(y1=abnormal_label, y2=np_decision)
            settings = config.to_string()
            insert_sql = """INSERT into model (model_name, pid, settings, dataset, file_name, TN, FP, FN, TP, precision, recall, fbeta, pr_auc, roc_auc, cks) 
                            VALUES('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')""" \
                .format('STL', config.pid, settings, config.dataset, Path(file_name).stem, cm[0][0], cm[0][1], cm[1][0], cm[1][1], precision, recall, fbeta, pr_auc, roc_auc, cks)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--robust', type=str2bool, default=1)
    ...
    if args.load_config:
        config = STLConfig(dataset=None, x_dim=None, period=None, robust=None, display=None, save_output=None,
                           save_figure=None, use_spot=None, save_config=None, load_config=None, server_run=None,
                           robustness=None, pid=None)
        try:
            config.import_config('./save_configs/{}/Config_STL_pid={}.json'.format(config.dataset, config.pid))
        except:
            print('There is no config.')
    else:
        config = STLConfig(dataset=args.dataset, x_dim=args.x_dim, period=args.period, robust=args.robust,
                           display=args.display, save_output=args.save_output, save_figure=args.save_figure,
    ...
    if args.save_config:
        ...
        config.export_config('./save_configs/{}/Config_STL_pid={}.json'.format(config.dataset, config.pid))
    ...
    train_logger, file_logger, meta_logger = create_logger(dataset=args.dataset,
                                                           train_logger_name='stl_train_logger',
                                                           file_logger_name='stl_file_logger',
                                                           meta_logger_name='stl_meta_logger',
                                                           model_name='STL',
                                                           pid=args.pid)
        ...
        if args.server_run:
            try:
                ...
                send_email_notification(
                    subject='baseline_stl application CRASHED! dataset error: {}, file error: {}, '
    ...
    if args.server_run:
        send_email_notification(
            subject='baseline_stl application FINISHED! dataset: {}, pid: {}'.format(args.dataset, args.pid),
            message='baseline_stl application FINISHED! dataset: {}, pid: {}'.format(args.dataset, args.pid))