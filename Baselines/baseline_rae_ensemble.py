...
class Encoder(nn.Module):
    def __init__(self, x_dim, h_dim, sequence_length, drop_connections, max_memory_steps, use_bidirection=False, batch_size=1, rnn_layers=1, ):
        ...
        self.memory_steps = torch.randint(1, max_memory_steps, (1,))[0]
        self.drop_connections = drop_connections
        
    def forward(self, input):
        h_i = Variable(torch.zeros(input.shape[0], self.h_dim), requires_grad=True).to(device)

        dropping_prob = torch.tensor([1-self.drop_connections, self.drop_connections], dtype=torch.float)
        hidden_states = torch.empty(1, 1)
        
        for i in range(input.shape[1]):
            if i > self.memory_steps:
                keep_last = dropping_prob.multinomial(1, replacement=False)
                if keep_last == 0:
                    keep_memory = dropping_prob.multinomial(1, replacement=False)
                    if keep_memory == 0:
                        h_i = self.gru_cell(input[:, i], (h_i + hidden_states[-self.memory_steps]))
                    else:
                        h_i = self.gru_cell(input[:, i], (h_i))
                else:
                    h_i = self.gru_cell(input[:, i], (hidden_states[-self.memory_steps]))
            else:    
                h_i = self.gru_cell(input[:, i], (h_i))
            
            if i == 0:
                hidden_states = torch.unsqueeze(h_i, 0)
            else:
                hidden_states = torch.cat([hidden_states,torch.unsqueeze(h_i, 0)],0)
        ...
        for i in range(input.shape[1]):
            if i > self.memory_steps:
                ...
                if keep_last == 0:
                    keep_memory = dropping_prob.multinomial(1, replacement=False)
                    if keep_memory == 0:
                        h_i = self.gru_cell(output[i - 1], (h_i + hidden_states[-self.memory_steps]))
                    else:
                        h_i = self.gru_cell(output[i - 1], (h_i))
                else:
                    h_i = self.gru_cell(output[i - 1], (hidden_states[-self.memory_steps]))
            elif i == 0:    
...
class RAE(nn.Module):
    def __init__(self, file_name, config):
        ...
        self.drop_connections = config.drop_connections
        self.max_memory_steps = config.max_memory_steps
        ...
        # units
        self.encoder = Encoder(x_dim=self.x_dim, h_dim=self.h_dim, sequence_length=self.rolling_size, use_bidirection=False, batch_size=self.batch_size, rnn_layers=self.rnn_layers, drop_connections=self.drop_connections, max_memory_steps= self.max_memory_steps)
        self.decoder = Decoder(x_dim=self.x_dim, h_dim=self.h_dim, sequence_length=self.rolling_size, use_bidirection=False, batch_size=self.batch_size, rnn_layers=self.rnn_layers, drop_connections=self.drop_connections, max_memory_steps= self.max_memory_steps)
    ...
    def fit(self, train_input, train_label, test_input, test_label, abnormal_data, abnormal_label, original_x_dim):
        ...
        with torch.no_grad():
            ...
            execution_time[np.where(execution_time == 0)[0][0]] = time.time() - start_time_file
            
            return rae_output

class RAE_Ensemble(nn.Module):
    def __init__(self, file_name, config):
        super(RAE_Ensemble, self).__init__()
        self.rae = RAE(file_name=file_name, config=config)
    ...
    def fit(self, train_input, train_label, test_input, test_label, abnormal_data, abnormal_label, original_x_dim):
        for member in range(self.members):
            self.outputs.append(
                self.rae.fit(train_input, train_label, test_input, test_label, abnormal_data, abnormal_label,
                             original_x_dim))
...
def RunModel(file_name, config):
    ...
    model = RAE_Ensemble(file_name=file_name, config=config)
    ...
    if train_data is not None and config.robustness == False:
        ...
        rae_output = model.fit(train_input=rolling_abnormal_data, train_label=rolling_abnormal_data,
                               ...
                               abnormal_data=abnormal_data, abnormal_label=abnormal_label, original_x_dim=original_x_dim)
    # %%
    rae_outputs = rae_output
    error_outputs = np.array([])
    
    for ensemble_index, rae_output in enumerate(rae_outputs):
        ...
        model_name = 'RAE_ENSEMBLE_' + str(ensemble_index)
        ...
        if ensemble_index == 0:
            ...
            error_outputs = np.vstack((error_outputs, error))
            error = np.median(error_outputs, axis = 0)
        ...
        if config.use_spot:
            ...
            try:
                ...
                if (ensemble_index + 1) == len(rae_outputs):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_memory_steps', type=int, default=10)
    parser.add_argument('--drop_connections', type=float, default=0.2)
    ...
    if args.load_config:
        config = RAEEnsembleConfig(dataset=None, x_dim=None, h_dim=None, preprocessing=None, use_overlapping=None,
                           ...
                           weight_decay=None, early_stopping=None, loss_function=None, rnn_layers=None, max_memory_steps=None,
                           ...
                           adjusted_points=None, use_best_f1=None, pid=None, use_slope=None, slope=None, ensemble_members=None, 
        ...
        config = RAEEnsembleConfig(dataset=args.dataset, x_dim=args.x_dim, h_dim=args.h_dim, preprocessing=args.preprocessing,
                           use_overlapping=args.use_overlapping, rolling_size=args.rolling_size, epochs=args.epochs,
                           milestone_epochs=args.milestone_epochs, lr=args.lr, max_memory_steps=args.max_memory_steps,
                           batch_size=args.batch_size, weight_decay=args.weight_decay, use_slope=args.use_slope,
                           early_stopping=args.early_stopping, loss_function=args.loss_function, slope=args.slope,
                           use_clip_norm=args.use_clip_norm, gradient_clip_norm=args.gradient_clip_norm,
                           rnn_layers=args.rnn_layers, use_bidirection=args.use_bidirection, gamma=args.gamma,
                           ...
                           save_model=args.save_model, load_model=args.load_model, ensemble_members=args.ensemble_members,
            ...
            if len(dirs) == 0:
                ...
                for file in files:
                    ...
                    if args.server_run:
                        ...
                        execution_time = np.zeros(config.ensemble_members)
                        metrics_result = RunModel(file_name=file_name, config=config)
                        s_TN.append(metrics_result.TN)
                        file_logger.info('TN = {}'.format(metrics_result.TN))
                        s_FP.append(metrics_result.FP)
